...
const ActionDrawer = ({ initiativeDetail, initiativesID, data, open, onClose, setRefetch, refetch, acc }) => {  
  ...
  // Added by Gauri for setting document list on 28 Feb 2025
  const [ActionDocList, setActionDocList] = useState([]);
  ...
  // Added state for requestStageId by Gauri on 12 Mar 2025
  const [requestStageID, setRequestStageID] = useState(initiativeDetail?.data?.listInitiativeDetailEntity[0].controlValue);
  console.log("requestStageID in ActionDrawer", requestStageID);
  ...
  // Modified by Gauri to fetch Document list on 28 Feb 2025
  const fetchActionDocumentList = async () => {
    ...
    const response = await fetch(
      ...
      `${process.env.REACT_APP_BASEURL_ACCESS_CONTROL1}/api/InitiativeList/GetActionItemDocumentList?ActionItemId=${data?.actionID}&UserId=${employeeId}`,
      {
        method: "GET",
        headers: {
    ...
    if (response.ok) {
      ...
      // setActionDocList(data.data.listDocumentCategory);
      setActionDocList(data.data.listActionItemDocumentListEntity);
      console.log("AI documentCategories", ActionDocList);
  ...
  const handleSave = async () => {
    ...
    const dataToSave = {
      ...
      // Commented and Added by Gauri to get requestStageId on 12 Mar 2025
          ...
          {formValues?.actionID && (
            <PivotItem headerText="Document Upload" itemKey="upload">
              <DocumentsComponentA
                ...
                initiativeDocument={ActionDocList}
                fetchActionDocumentList={fetchActionDocumentList} 
