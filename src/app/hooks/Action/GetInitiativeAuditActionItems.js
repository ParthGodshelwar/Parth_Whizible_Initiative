...
const GetInitiativeAuditActionItems = async (IdeaId, currentPage, formValues) => {
  const accessToken = sessionStorage.getItem("access_token");
  const userdata = JSON.parse(sessionStorage.getItem("user"));
  const employeeId = userdata?.employeeId;
  console.log("currentPage88", currentPage);

  // Added by Gauri to format date while filtering data by date on 06 Mar 2025
  ...
  try {
    ...
    const filteredParams = Object.entries({
      ...
      // Commented and Added by Gauri to format SubmittedOn and DueDate on 06 Mar 2025
    ...
    const response = await axios.get(
      `${process.env.REACT_APP_BASEURL_ACCESS_CONTROL1}/api/InitiativeList/GetInitiativeAuditActionItems`,
      {
...
export default GetInitiativeAuditActionItems;
